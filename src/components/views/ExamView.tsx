...
import { 
  ...
  Award, Download, Printer, Scissors, Layers, MinusCircle, XCircle
} from 'lucide-react';
...
export const ExamView = ({ exam, onComplete, onCancel, user, moduleName }: ExamViewProps) => {
  ...
  const [answers, setAnswers] = useState<any[]>(() => {
    ...
    return questions.map(q => {
      ...
      if (q.type === 'fill-in-the-blanks') {
        const numBlanks = (q.text || '').split(/\[blank\]/g).length - 1;
        const size = numBlanks > 0 ? numBlanks : ((q.correctAnswers && q.correctAnswers.length) || 0);
  ...
  const isQuestionAnswered = (idx: number): boolean => {
    ...
    if (q.type === 'matching') {
      if (!Array.isArray(ans)) return false;
      const totalMatches = (q.options || []).length;
      if (totalMatches === 0) return true;
      for (let i = 0; i < totalMatches; i++) {
        const v = ans[i];
        if (v === undefined || v === null || v === -1 || String(v).trim() === '') {
          return false;
        }
      }
      return true;
    ...
    if (Array.isArray(ans)) {
      return ans.length > 0 && ans.every(v => v !== -1 && v !== '' && v !== null && String(v).trim() !== '');
    }

    return ans !== -1 && ans !== '' && ans !== null && String(ans).trim() !== '';
  };
  ...
  const [finalResult, setFinalResult] = useState<{ score: number, totalPoints: number, aiFeedback?: string, questionResults?: any[] } | null>(null);
  const [activeResultTab, setActiveResultTab] = useState<'overview' | 'correction' | 'ai_analysis' | 'attestation'>('overview');
  const [correctionFilter, setCorrectionFilter] = useState<'all' | 'correct' | 'incorrect'>('all');
  ...
  useEffect(() => {
    ...
    const ansCount = questions.filter((_, idx) => isQuestionAnswered(idx)).length;
                ...
                <div className="p-8 md:p-12 space-y-10">
                  ...
                  <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between border-b border-slate-100 pb-6 gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto shrink-0">
                      ...
                      <div className={cn(
                        "px-5 py-3 rounded-2xl flex items-center justify-center gap-3 border shadow-sm shrink-0",
                      ...
                      <div className="flex flex-col sm:flex-row gap-2.5 w-full sm:w-auto">
                        <button
                          ...
                          className="px-5 py-3.5 rounded-xl border-2 border-indigo-600 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[10px] tracking-widest transition-all flex items-center justify-center gap-2 active:scale-98 shadow-md cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="px-5 py-3.5 rounded-xl border-2 border-slate-200 text-slate-700 bg-white hover:bg-slate-50 hover:border-indigo-200 hover:text-indigo-650 font-black uppercase text-[10px] tracking-widest transition-all flex items-center justify-center gap-2 active:scale-98 cursor-pointer"
                        >
                        ...
                        <Button 
                          onClick={() => onComplete()}
                          className="px-6 py-3.5 h-auto bg-slate-950 hover:bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.15em] shadow-md transition-all flex items-center justify-center gap-2 group cursor-pointer"
                        >
                          <span>Quitter</span>
                  ...
                  <div className="flex border-b border-indigo-150 overflow-x-auto h-12 custom-scrollbar gap-2 sm:gap-6 mt-4">
                    {[
                      { id: 'overview', label: 'Synthèse & Performance', icon: ClipboardList },
                      { id: 'correction', label: 'Correction de l\'Épreuve', icon: CheckCircle2 },
                      { id: 'ai_analysis', label: 'Conseils Éducatifs de l\'IA', icon: Sparkles },
                      { id: 'attestation', label: 'Attestation & Certificat', icon: Award },
                    ].map((t) => {
                      const Icon = t.icon;
                      const isSelected = activeResultTab === t.id;
                      return (
                        <button
                          key={t.id}
                          onClick={() => setActiveResultTab(t.id as any)}
                          className={cn(
                            "flex items-center gap-2 px-4 py-2 text-xs font-black uppercase tracking-wider border-b-2 transition-all shrink-0 cursor-pointer",
                            isSelected
                  ...
                  <div className="mt-6">
                    ...
                    {activeResultTab === 'overview' && (
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fadeIn">
                        ...
                        {/* Left Column: Visual circular progress */}
                        <div className="lg:col-span-12 xl:col-span-5 flex flex-col items-center">
                          <div className="bg-slate-50/50 w-full p-8 rounded-[2rem] border border-slate-100 flex flex-col items-center relative overflow-hidden">
                            <div className="relative flex items-center justify-center">
                              ...
                              <div className="absolute top-2 right-2 bg-indigo-600 p-2.5 rounded-2xl shadow-lg shadow-indigo-100 transform rotate-12">
                                <Star className="w-5 h-5 text-white fill-white animate-bounce" />
                              </div>
                            </div>

                            <div className="text-center mt-6 space-y-2">
                              <h4 className="text-xl font-black text-slate-900 tracking-tight font-display">
                              ...
                              <p className="text-xs text-slate-550 font-bold font-sans">
                        ...
                        <div className="lg:col-span-12 xl:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
                          ...
                          <div className={cn(
                            "p-6 rounded-[2rem] border flex flex-col justify-between h-[160px] relative overflow-hidden shadow-xs",
                            ...
                            <div className="flex justify-between items-start">
                              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Décision d'Admission</span>
                              <div className={cn(
                            ...
                            <div>
                              <p className="text-2xl font-black font-display uppercase tracking-tight">
                                {Math.round((finalResult.score / finalResult.totalPoints) * 100) >= 50 ? "Admis" : "Échec d'épreuve"}
                              </p>
                              <p className="text-[10px] opacity-75 font-semibold mt-1 font-sans">
                                {Math.round((finalResult.score / finalResult.totalPoints) * 100) >= 50 ? "Félicitations, vous validez ce module d'apprentissage!" : "Nous vous encourageons à ré-essayer après avoir révisé."}
                              </p>
                            </div>
                          </div>

                          {/* Card 2: focus details */}
                          <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 flex flex-col justify-between h-[160px] shadow-xs">
                            <div className="flex justify-between items-start">
                              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Qualité de Focus & Surveillance</span>
                            ...
                            <div>
                              <p className="text-2xl font-black text-slate-900 font-display">
                                {tabExitCount === 0 && fullscreenExitsCount === 0 ? "Excellente" : `${tabExitCount + fullscreenExitsCount} avert.`}
                              </p>
                              <p className="text-[10px] text-slate-550 font-bold mt-1 font-sans">
                                {tabExitCount === 0 && fullscreenExitsCount === 0 ? "Aucun changement d'onglet ou sortie d'écran détecté." : `L'élève est sorti de l'onglet ${tabExitCount} fois et du plein écran ${fullscreenExitsCount} fois.`}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 flex flex-col justify-between h-[160px] shadow-xs">
                            <div className="flex justify-between items-start">
                              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 font-sans">Durée Totale Active</span>
                              <div className="w-10 h-10 rounded-2xl bg-slate-150 flex items-center justify-center text-slate-600 shadow-xs">
                            ...
                            <div>
                              <p className="text-2xl font-black text-slate-900 font-display">
                                {(() => {
                                  const totalAllowed = (exam.durationMinutes + extraTimeMinutes) * 60;
                                  const remaining = timeLeft;
                                  const used = Math.max(0, totalAllowed - remaining);
                              ...
                              <p className="text-[10px] text-slate-400 font-bold mt-1 font-sans">
                                Temps alloué total : {exam.durationMinutes + extraTimeMinutes} min.
                              </p>
                            </div>
                          </div>

                          {/* Card 4: Stats breakdown */}
                          <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 flex flex-col justify-between h-[160px] shadow-xs">
                            <div className="flex justify-between items-start">
                              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 font-sans">Questions & Composition</span>
                              <div className="w-10 h-10 rounded-2xl bg-slate-150 flex items-center justify-center text-slate-600 shadow-xs font-sans">
                                <Layers className="w-5 h-5" />
                              </div>
                            </div>
                            <div>
                              <p className="text-2xl font-black text-slate-900 font-display">
                                {questions.length} / {exam.questions.length} Questions
                              </p>
                              <p className="text-[10px] text-slate-400 font-bold mt-1 font-sans">
                                {exam.questions.filter(q => q.type === 'multiple-choice').length} QCM • {exam.questions.filter(q => q.type === 'true-false').length} V/F • {exam.questions.filter(q => q.type === 'short-answer').length} Courte.
                              </p>
                            </div>
                          </div>
                    ...
                    {/* AI ANALYSIS TAB */}
                    {activeResultTab === 'ai_analysis' && (
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fadeIn">
                        <div className="lg:col-span-8 space-y-6">
                          <div className="bg-indigo-50/40 rounded-3xl p-6 border border-indigo-100/50 space-y-4 font-sans shadow-xs">
                            <div className="flex items-center justify-between">
                              ...
                              <button 
                                ...
                                className="text-xs font-black text-indigo-600 hover:text-indigo-800 transition-colors uppercase tracking-wider animate-pulse cursor-pointer"
                              >
                                Copier l'analyse
                              </button>
                            </div>
                            
                            <div className="text-sm text-slate-700 font-medium leading-relaxed max-h-[400px] overflow-y-auto custom-scrollbar pr-2 whitespace-pre-wrap bg-white/60 p-5 rounded-2xl border border-indigo-100/30">
                              {finalResult.aiFeedback || "Aucune remarque spécifique n'a été formulée par l'intelligence artificielle pour cette épreuve."}
                            </div>
                          </div>
                        </div>

                        {/* Side prompt to assist AI feedback understanding */}
                        <div className="lg:col-span-4 space-y-6">
                          <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-6 rounded-[2rem] border border-indigo-950 shadow-xl space-y-4">
                            <h4 className="text-base font-black uppercase tracking-wider font-display">Analyse Formative IA</h4>
                            <p className="text-xs text-indigo-200 font-medium font-sans leading-relaxed">
                              Notre moteur d'intelligence artificielle analyse l'ensemble de votre prestation. Il identifie les schémas d'erreurs, les biais décisionnels, et suggère des actions correctives individualisées pour renforcer votre socle académique.
                            </p>
                            <div className="pt-4 border-t border-indigo-800/65 space-y-2">
                              <p className="text-[10px] text-indigo-300 font-black uppercase tracking-widest font-sans">Conseils d'Apprentissage</p>
                              <p className="text-xs text-slate-50 font-bold font-sans animate-pulse">
                                Relisez attentivement le feedback détaillé par question ci-dessus pour comprendre les motifs correctifs.
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    {/* ATTESTATION TAB */}
                    {activeResultTab === 'attestation' && (
                      <div className="space-y-6 animate-fadeIn">
                        
                        {/* Summary details */}
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div>
                            <h4 className="text-lg font-black text-slate-950 font-display uppercase tracking-tight italic">
                              Attestation Officielle de Fin de Module
                            </h4>
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-0.5 font-sans">
                              Visualisez ou exportez votre diplôme certifié d'évaluation académique.
                            </p>
                          </div>
                        </div>
                        ...
                        {exam.questions.some(q => q.type === 'practical') && (
                          <div className="flex items-start gap-4 p-5 bg-amber-50/60 border border-amber-100/60 rounded-3xl text-amber-900 shadow-xs">
                            <Info className="w-5 h-5 text-amber-600 shrink-0 mt-0.5 animate-pulse" />
                            <p className="text-[11px] font-bold leading-normal italic font-sans">
                        ...
                        {/* Interactive Certificate Preview Layout */}
                        <div className="bg-amber-50/10 border-2 border-dashed border-amber-500/20 rounded-[2.5rem] p-6 sm:p-8 space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[9px] font-black uppercase text-amber-700 bg-amber-50 px-2.5 py-1 rounded-xl border border-amber-250/50 font-sans">
                              {Math.round((finalResult.score / finalResult.totalPoints) * 100) >= 50 ? 'Réussite' : 'Passage'}
                            </span>
                          </div>
                          
                          <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm bg-slate-50 p-2 md:p-3 max-h-[350px] overflow-y-auto custom-scrollbar">
                            <AttestationTemplate 
                              exam={exam} 
                              result={finalResult} 
                              user={user} 
                          ...
                          <p className="text-[8px] text-slate-400 font-extrabold text-center uppercase tracking-widest leading-none font-sans mt-2">
                  ...
                  {activeResultTab === 'correction' && (
                    <div className="space-y-6 pt-2 animate-fadeIn">
                      
                      {/* Correction Sub-Filters */}
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                        <div>
                          <h4 className="text-xl font-black text-slate-950 font-display uppercase tracking-tight italic">
                            Correction Détallée de l'Épreuve
                          </h4>
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-0.5 font-sans">
                            Consultez vos réponses ainsi que les explications et réponses attendues.
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl w-fit border border-slate-200">
                          {[
                            { id: 'all', label: 'Toutes' },
                            { id: 'correct', label: 'Correctes' },
                            { id: 'incorrect', label: 'Incorrectes' }
                          ].map((f) => {
                            const isSelected = correctionFilter === f.id;
                            return (
                              <button
                                key={f.id}
                                onClick={() => setCorrectionFilter(f.id as any)}
                                className={cn(
                      ...
                      <div className="space-y-6">
                        {questions.map((q, idx) => {
                          const qRes = finalResult.questionResults?.[idx];
                          const pointsEarned = qRes?.pointsEarned || 0;
                          const isCorrect = pointsEarned === (q.points || 1);
                          const isPartial = pointsEarned > 0 && pointsEarned < (q.points || 1);
                          const studentAns = answers[idx];
                          const qAr = isArabic(q.text || '');

                          // Apply correct/incorrect filters
                          if (correctionFilter === 'correct' && !isCorrect) return null;
                          if (correctionFilter === 'incorrect' && isCorrect) return null;
                          ...
                          <div
                            ...
                            dir={qAr ? "rtl" : "ltr"}
                          >
                            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-dashed border-slate-100 pb-4">
                              <div className="flex items-start gap-4">
                                <span className="w-8 h-8 rounded-xl bg-white border border-slate-150 flex items-center justify-center text-xs font-black text-slate-450 shrink-0 shadow-sm font-mono text-slate-500">
                                  #{idx + 1}
                                </span>
                                <div className="space-y-2">
                                  <div className="flex flex-wrap items-center gap-1.5 leading-none">
                                    ...
                                    <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 uppercase tracking-widest">
                                      {q.type}
                                    </span>
                                    <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 uppercase tracking-widest">
                                      {q.points || 1} Pts
                                    </span>
                                  </div>
                                  <div 
                                    className="text-sm font-bold text-slate-900 leading-relaxed font-sans"
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                <div className={cn(
                                  "flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-slate-150 shadow-sm",
                                  isCorrect ? "text-emerald-600 border-emerald-100" : isPartial ? "text-amber-600 border-amber-105" : "text-rose-600 border-rose-100"
                                )}>
                                  {isCorrect ? (
                            ...
                            <div className="space-y-4 pt-1 sm:pl-12 pr-4">
                              {/* Option list with visual user answer representation */}
                              {q.type === 'matching' && q.runtimeMatchOptions && (
                                <div className="space-y-2 font-sans">
                                  {(q.options || []).map((leftOpt, lIdx) => {
                                    const matchedRightIdx = studentAns?.[lIdx];
                                    const rightText = (matchedRightIdx !== undefined)
                                      ? q.runtimeMatchOptions.find(ro => ro.idx === matchedRightIdx)?.text
                                      : null;
                                    const isMatchCorrect = matchedRightIdx === q.correctMatches?.[lIdx];
                                    return (
                                      <div key={lIdx} className={cn(
                                        "p-3 rounded-xl border text-xs font-bold flex flex-wrap justify-between items-center bg-white border-slate-100",
                                        isMatchCorrect ? "text-emerald-700" : "text-rose-700"
                                      )}>
                                        <div className="flex items-center gap-3">
                                          <span className="opacity-70" dangerouslySetInnerHTML={{ __html: leftOpt.text }} />
                                          <span className="text-slate-300">→</span>
                                          <span className="font-extrabold" dangerouslySetInnerHTML={{ __html: rightText || 'Non associé' }} />
                                        </div>
                                        {!isMatchCorrect && (
                                          <span className="text-[9px] font-black uppercase text-slate-400">
                                            Correct: {q.matchOptions?.[q.correctMatches?.[lIdx] as any]}
                                          </span>
                                        )}
                                      </div>
                                    );
                              ...
                              {q.type === 'ordering' && q.runtimeOptions && (
                                <div className="space-y-2 font-sans">
                                  <div className="flex flex-wrap gap-2">
                                    {(studentAns as number[] || []).map((optIdx, sortedIdx) => {
                                      const opt = q.runtimeOptions.find(ro => ro.idx === optIdx);
                                      const isPosCorrect = optIdx === q.correctOrder?.[sortedIdx];
                                      return (
                                        <div key={sortedIdx} className={cn(
                                          "flex items-center gap-2 p-2 rounded-lg border text-[10px] font-bold bg-white",
                                          isPosCorrect ? "text-emerald-700 border-emerald-100 bg-emerald-50/20" : "text-rose-700 border-rose-100 bg-rose-50/20"
                                        )}>
                                          <span className="w-5 h-5 flex items-center justify-center rounded bg-slate-100 font-black text-[8px] font-mono">
                                            {sortedIdx + 1}
                                          </span>
                                  ...
                                  {!isCorrect && (
                                    <div className="space-y-1 mt-2">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        {q.correctOrder?.map((optIdx, sortedIdx) => (
                                          <div key={sortedIdx} className="flex items-center gap-2 p-2 rounded-lg border border-emerald-100 bg-emerald-50/30 text-[10px] font-bold text-emerald-700 opacity-70">
                                            <span className="w-5 h-5 flex items-center justify-center rounded bg-white shadow-xs font-black text-[8px] font-mono">
                                              {sortedIdx + 1}
                                            </span>
                              ...
                              {q.type === 'fill-in-the-blanks' && (
                                <div className="p-3.5 bg-slate-50/30 rounded-xl border border-slate-150/45 text-slate-800 text-xs font-medium leading-relaxed font-sans">
                                  {q.text.split(/\[blank\]/g).map((part, pIdx, arr) => (
                                    <React.Fragment key={pIdx}>
                                      <span dangerouslySetInnerHTML={{ __html: part }} />
                                      {pIdx < arr.length - 1 && (
                                        <span className={cn(
                                          "inline-block mx-1 px-2.5 py-0.5 rounded border-b-2 font-black text-xs",
                                          studentAns?.[pIdx] === q.correctAnswers?.[pIdx] 
                              ...
                              {q.type !== 'matching' && q.type !== 'ordering' && q.type !== 'fill-in-the-blanks' && (
                                <div className="space-y-2 font-sans">
                                  ...
                                  <div className="p-3 sm:p-4 bg-white/80 rounded-xl border border-slate-150/60 text-xs sm:text-sm font-bold text-indigo-600 break-words">
                              ...
                              {/* Correct Answer Reference (Always shown for incorrect student answers) */}
                              {!isCorrect && (
                                <div className="space-y-2 font-sans">
                                  <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Réponse Correcte Attendue</p>
                                  <div className="p-3 sm:p-4 bg-emerald-50/50 rounded-xl border border-emerald-100 text-xs sm:text-sm font-bold text-emerald-700 break-words">
                                    {q.type === 'multiple-choice' || q.type === 'true-false' ? (
              ...
              <div className="space-y-8">
                {questions.map((q, idx) => {
                  ...
                  return (
                    <div
                      key={q.id}
                      className="p-8 rounded-[2.5rem] border-2 border-slate-100 bg-slate-50/30 hover:border-indigo-100/50 hover:bg-white transition-all space-y-6 relative group"
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            isAnswered ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"
                          )}>
                            {idx + 1}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Question {idx + 1}</span>
                              <span className="text-[9px] font-black px-2 py-0.5 rounded bg-slate-100 text-slate-500 uppercase tracking-widest">{q.type}</span>
                              <span className="text-[9px] font-black px-2 py-0.5 rounded bg-indigo-50 text-indigo-600 uppercase tracking-widest">{q.points} Pts</span>
                            </div>
                            <p className={cn(
                              "text-xs font-bold leading-none mt-1",
                              isAnswered ? "text-emerald-600" : "text-amber-500"
                            )}>
                              {isAnswered ? "✓ Répondu" : "⚠ Non répondu"}
                            </p>
                          </div>
                        </div>
                        ...
                        <Button
                          ...
                          className="rounded-xl border-slate-200 hover:border-indigo-600 hover:text-indigo-600 font-bold text-xs h-9"
                        >
                          <NotebookPen className="w-3.5 h-3.5 mr-1" /> Modifier la réponse
                        </Button>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div 
                          className={cn(
                            "text-base font-black text-slate-900 leading-relaxed max-w-none first-letter:capitalize",
                        ...
                        {/* Render Student's Selected Answer State */}
                        {isAnswered ? (
                          <div className="mt-4">
                            {/* Multiple Choice Options List with highlight on Student's Selection */}
                            {q.type === 'multiple-choice' && q.runtimeOptions && (
                              <div className="space-y-2.5">
                                {q.runtimeOptions.map((opt, oIdx) => {
                                  const isSelected = answers[idx] === opt.idx;
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className={cn(
                                        "w-7 h-7 rounded-lg flex items-center justify-center font-black text-xs shrink-0 transition-colors",
                                        isSelected ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-400"
                                      )}>
                                        {String.fromCharCode(65 + oIdx)}
                                      </div>
                                      <div className="flex-1 text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: opt.text }} />
                            ...
                            {q.type === 'true-false' && (
                              <div className="flex gap-4">
                                {[
                                  ...
                                  const isSelected = answers[idx] === choice.val;
                                  return (
                                    <div 
                                      key={choice.val} 
                                      className={cn(
                                        "flex-1 p-4 rounded-xl border-2 font-black text-center flex items-center justify-center gap-2 transition-all text-sm",
                                        isSelected 
                                          ? choice.val === 0 
                                    ...
                                    >
                                      {choice.val === 0 ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                            ...
                            {q.type === 'short-answer' && (
                              <div className="space-y-1.5">
                                <div className="text-[9px] uppercase font-black text-slate-400 tracking-widest">Votre Réponse :</div>
                                <div className="p-5 rounded-2xl border border-slate-200 bg-white shadow-inner text-slate-800 text-sm font-medium whitespace-pre-wrap leading-relaxed">
                                  {answers[idx]}
                                </div>
                              </div>
                            )}

                            {/* Fill in the blanks reconstituted text */}
                            {q.type === 'fill-in-the-blanks' && (
                              <div className="space-y-1.5">
                                <div className="text-[9px] uppercase font-black text-slate-400 tracking-widest">Texte complété :</div>
                                <div className="p-5 rounded-2xl border border-indigo-100 bg-indigo-50/20 text-slate-800 text-sm font-medium leading-relaxed">
                                  {q.text.split(/\[blank\]/g).map((part, pIdx, arr) => (
                                    <React.Fragment key={pIdx}>
                                      <span dangerouslySetInnerHTML={{ __html: part }} />
                                      {pIdx < arr.length - 1 && (
                                        <span className="mx-1 px-2.5 py-1 rounded bg-indigo-600 text-white font-black inline-block text-xs shadow-sm">
                                          {answers[idx]?.[pIdx] || '...'}
                                        </span>
                                      )}
                                    </React.Fragment>
                            ...
                            {q.type === 'ordering' && q.runtimeOptions && (
                              <div className="space-y-1.5">
                                <div className="text-[9px] uppercase font-black text-slate-400 tracking-widest">Votre Ordre Retenu :</div>
                                <div className="space-y-2">
                                  {(answers[idx] as number[] || []).map((optIdx, sortedIdx) => {
                                    const opt = q.runtimeOptions.find(ro => ro.idx === optIdx);
                                    return (
                                      <div key={sortedIdx} className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
                                        <div className="w-6 h-6 rounded bg-indigo-600 text-white flex items-center justify-center font-black text-xs shrink-0">
                                          {sortedIdx + 1}
                                        </div>
                                        <div className="text-xs font-bold text-slate-700" dangerouslySetInnerHTML={{ __html: opt?.text || '' }} />
                                      </div>
                                    );
                                  })}
                                </div>
                            ...
                            {q.type === 'matching' && q.runtimeMatchOptions && (
                              <div className="space-y-1.5">
                                <div className="text-[9px] uppercase font-black text-slate-400 tracking-widest">Vos Associations :</div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                  {(q.options || []).map((leftOpt, lIdx) => {
                                    const matchedRightIdx = answers[idx]?.[lIdx];
                                    const rightText = (matchedRightIdx !== undefined)
                                      ? q.runtimeMatchOptions.find(ro => ro.idx === matchedRightIdx)?.text
                                      : null;
                                    return (
                                      <div key={lIdx} className="p-3 bg-white border border-slate-100 rounded-xl flex flex-col justify-between shadow-sm">
                                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Élément {lIdx + 1}</div>
                                        <div className="text-xs font-bold text-slate-800" dangerouslySetInnerHTML={{ __html: leftOpt.text }} />
                                        <div className="text-xs text-indigo-700 font-bold border-t border-indigo-100/30 mt-2 pt-2 flex items-center gap-1">
                                          <span>→ matched with:</span>
                                          {rightText ? (
                                            <strong dangerouslySetInnerHTML={{ __html: rightText }} />
                                          ) : (
                                            <span className="text-rose-500 italic">Vide</span>
                                          )}
                                        </div>
                                      </div>
                                    );
                          ...
                          <div className="p-4 bg-amber-50/50 border border-amber-100 rounded-2xl flex items-center gap-2.5 text-amber-700 text-xs font-bold">
                            ...
                            {i < arr.length - 1 && (
                              <motion.input
                                ...
                                onChange={(e) => {
                                  ...
                                  } else if (newAns.length !== arr.length - 1) {
                                    const adjusted = new Array(arr.length - 1).fill('');
                                    for (let k = 0; k < Math.min(newAns.length, arr.length - 1); k++) {
                                      adjusted[k] = newAns[k];
                                    }
                                    newAns = adjusted;
                                  }
                          ...
                          <button 
                            onClick={async () => {
                              const ok = await confirm({
                                ...
                                message: "Voulez-vous effacer l'intégralité de ce brouillon ?",
                          ...
                          <button 
                            onClick={async () => {
                              const ok = await confirm({
                                ...
                                message: "Voulez-vous effacer les notes de cette question ?",
                                confirmLabel: "Effacer",
                                cancelLabel: "Annuler",
                                variant: "danger"
                              });
